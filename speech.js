...
const mapping_robo = {
    "hallo": 'Wave', "bye": 'ThumbsUp', "tschüss": 'ThumbsUp', "sitzen": "Sitting", "stehen": "Standing",
...
recognition.onresult = function (event) {
    ...
    var hello = ["hallo","hey"]
    var das_ist = ["das", "ist"]
    var clampWhenFinished = false
    if (new RegExp(das_ist.join('|')).test(word)) {
        repetitions = 1
        animation = mapping_robo["tot"]
        emotion = {"morphtarget": "Sad", "value": 0}
        clampWhenFinished=true
    }
    else if (new RegExp(hello.join('|')).test(word)) {
        repetitions = 3
        animation = mapping_robo["hallo"]
        emotion = {"morphtarget": "Sad", "value": 0}
    }
    else if (new RegExp(no.join('|')).test(word)) {
        repetitions = 1
        ...
        emotion = {"morphtarget": "Surpise", "value": 4}
    ...
    nextAnimation(animation, emotion, repetitions,clampWhenFinished)
...
function nextAnimation(animation, emotion, repetitions,clampWhenFinished) {
